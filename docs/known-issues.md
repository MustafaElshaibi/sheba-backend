...
| ~~T-NOT-1~~ | ~~`Notification`/`NotificationTemplate` entities are TODO stubs; sends are hardcoded strings~~ | ~~No bilingual templating~~ | Resolved — bilingual templates seeded; Identity handlers use `INotificationTemplateService` |
...
| ~~T-WAL-1~~ | ~~VC signing key is ephemeral; no production fail-fast~~ | ~~Issued VCs silently invalidated~~ | Resolved — `AddWalletModule` throws `InvalidOperationException` on startup in non-Development if key unset; ephemeral dev key logs a loud warning |
